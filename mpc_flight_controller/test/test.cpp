...
int main()
{
	...
	for (i = 0; i < NY * N; ++i)
	{
		...
		acadoVariables.y[i * NY + 2] = 5; // z
		acadoVariables.y[i * NY + 3] = 0; // psi
		acadoVariables.y[i * NY + 4] = 0; // theta
		acadoVariables.y[i * NY + 5] = 0; // vzd
	}

	acadoVariables.yN[ 0 ] = 0; // x
	acadoVariables.yN[ 1 ] = 0; // y
	acadoVariables.yN[ 2 ] = 5; // z
	...
	{
		cout << "----- online data -----" << endl;
		for(i = 0; i < NOD; ++i)
		{
			cout << acadoVariables.od[i] << " ";
		}
		cout << endl;
		t_pre = clock();
