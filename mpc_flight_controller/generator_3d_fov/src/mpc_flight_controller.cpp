...
int main( )
{
	...
	Function h, hN;
	h << psid << thetad << vzd << phird << sv_obs1 << sv_obs2 << sv_fov;
	hN << x << y << z << phi;
	...
	S_obs1(2,0) = 0;
	S_obs1(2,1) = 0;
	S_obs1(2,2) = 1 / ((obs1_c + 0.3) * (obs1_c + 0.3));
	...
	S_obs2(2,0) = 0;
	S_obs2(2,1) = 0;
	S_obs2(2,2) = 1 / ((obs2_c + 0.3) * (obs2_c + 0.3));
	...
	ocp.subjectTo( 0 <= tPlaneCheck + sv_fov );
	ocp.subjectTo( 0 <= bPlaneCheck + sv_fov );
	ocp.subjectTo( 0 <= lPlaneCheck + sv_fov );
	ocp.subjectTo( 0 <= rPlaneCheck + sv_fov );
