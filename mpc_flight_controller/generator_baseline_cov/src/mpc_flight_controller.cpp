...
int main( )
{
	...
	const float Ts   = 1 / 15.0;	// sampling time
	const float TEnd = Ts * 25;     // prediction duration
	const float c    = 0.9062;      // constant coefficient calculate from erfinv(1-2*delta)
	const float mav_r= 0.3;			// quadrotor radius
    // Variables:
	...
	OnlineData 			Sigma_q_x;	 
	OnlineData			Sigma_q_y;  
	OnlineData			Sigma_q_z;

	OnlineData 			Sigma_obs1_x;	 
	OnlineData			Sigma_obs1_y;  
	OnlineData			Sigma_obs1_z;

	OnlineData 			Sigma_obs2_x;	 
	OnlineData			Sigma_obs2_y;  
	OnlineData			Sigma_obs2_z;  
	...
	// Expression Sigma_q(3,3);
	// Sigma_q(0,0) = Sigma_q_x;
	// Sigma_q(0,1) = 0;
	// Sigma_q(0,2) = 0;
	// Sigma_q(1,0) = 0;
	// Sigma_q(1,1) = Sigma_q_y;
	// Sigma_q(1,2) = 0;
	// Sigma_q(2,0) = 0;
	// Sigma_q(2,1) = 0;
	// Sigma_q(2,2) = Sigma_q_z;
	...
	Expression Sigma_obs1(3,3);
	Sigma_obs1(0,0) = Sigma_q_x + Sigma_obs1_x;
	Sigma_obs1(0,1) = 0;
	Sigma_obs1(0,2) = 0;
	Sigma_obs1(1,0) = 0;
	Sigma_obs1(1,1) = Sigma_q_y + Sigma_obs1_y;
	Sigma_obs1(1,2) = 0;
	Sigma_obs1(2,0) = 0;
	Sigma_obs1(2,1) = 0;
	Sigma_obs1(2,2) = Sigma_q_z + Sigma_obs1_z;
	...
	S_obs1(0,0) = 1 / (obs1_a + mav_r );
	S_obs1(0,1) = 0;
	S_obs1(0,2) = 0;
	S_obs1(1,0) = 0;
	S_obs1(1,1) = 1 / (obs1_b + mav_r );
	S_obs1(1,2) = 0;
	S_obs1(2,0) = 0;
	S_obs1(2,1) = 0;
	S_obs1(2,2) = 1 / (obs1_c + mav_r );
	S_obs1 = R_obs1 * S_obs1;

	Expression p_q2obs1(3,1);
	p_q2obs1 = S_obs1 * v_q2obs1;

	Expression p_q2obs1_norm;
	p_q2obs1_norm = sqrt(p_q2obs1.transpose() * p_q2obs1);

	Expression Sigma_q2obs1(3,3);
	Sigma_q2obs1 = S_obs1.transpose() * Sigma_obs1 * S_obs1;
	
	Expression Eta_obs1;
	Eta_obs1 = c * sqrt(2 * p_q2obs1.transpose() * Sigma_q2obs1 * p_q2obs1) / p_q2obs1_norm;

	Expression pseudoDist1; // in < 1; out > 1; on = 1
	// pseudoDist1 = v_q2obs1.transpose() * S_obs1 * v_q2obs1 + sqrt(2 * p_q2obs1.transpose() * Sigma_q2obs1 * p_q2obs1);
	pseudoDist1 = p_q2obs1_norm - Eta_obs1;
	...
	Expression Sigma_obs2(3,3);
	Sigma_obs2(0,0) = Sigma_q_x + Sigma_obs2_x;
	Sigma_obs2(0,1) = 0;
	Sigma_obs2(0,2) = 0;
	Sigma_obs2(1,0) = 0;
	Sigma_obs2(1,1) = Sigma_q_y + Sigma_obs2_y;
	Sigma_obs2(1,2) = 0;
	Sigma_obs2(2,0) = 0;
	Sigma_obs2(2,1) = 0;
	Sigma_obs2(2,2) = Sigma_q_z + Sigma_obs2_z;
	...
	S_obs2(0,0) = 1 / (obs2_a + mav_r );
	S_obs2(0,1) = 0;
	S_obs2(0,2) = 0;
	S_obs2(1,0) = 0;
	S_obs2(1,1) = 1 / (obs2_b + mav_r );
	S_obs2(1,2) = 0;
	S_obs2(2,0) = 0;
	S_obs2(2,1) = 0;
	S_obs2(2,2) = 1 / (obs2_c + mav_r );
	S_obs2 = R_obs2 * S_obs2;

	Expression p_q2obs2(3,1);
	p_q2obs2 = S_obs2 * v_q2obs2;

	Expression p_q2obs2_norm;
	p_q2obs2_norm = sqrt(p_q2obs2.transpose() * p_q2obs2);

	Expression Sigma_q2obs2(3,3);
	Sigma_q2obs2 = S_obs2.transpose() * Sigma_obs2 * S_obs2;
	
	Expression Eta_obs2;
	Eta_obs2 = c * sqrt(2 * p_q2obs2.transpose() * Sigma_q2obs2 * p_q2obs2) / p_q2obs2_norm;

	Expression pseudoDist2; // in < 1; out > 1; on = 1
	// pseudoDist2 = v_q2obs2.transpose() * S_obs2 * v_q2obs2;
	pseudoDist2 = p_q2obs2_norm - Eta_obs2;
